...
PBL_APP_INFO(MY_UUID, "Roboto Big Animated", "Tom Svoboda", 2, 0, RESOURCE_ID_IMAGE_MENU_ICON, APP_INFO_WATCH_FACE);
...
const int IMAGE_RESOURCE_IDS[NUMBER_OF_IMAGES] = {
  ...
  RESOURCE_ID_IMAGE_THIN_0, RESOURCE_ID_IMAGE_THIN_1, RESOURCE_ID_IMAGE_THIN_2,
  RESOURCE_ID_IMAGE_THIN_3, RESOURCE_ID_IMAGE_THIN_4, RESOURCE_ID_IMAGE_THIN_5,
  RESOURCE_ID_IMAGE_THIN_6, RESOURCE_ID_IMAGE_THIN_7, RESOURCE_ID_IMAGE_THIN_8,
  RESOURCE_ID_IMAGE_THIN_9
};
	...
	*/
	...
	for (int column_number = 1; column_number >= 0; column_number--) {
		...
		if ((value == 0) && (column_number == 0) && !show_first_leading_zero) { //if number shouldn't be present on the screen
			...
			check_and_change((value % 10)+10, slot_number, false);
		}
		value = value / 10;
	}
}
